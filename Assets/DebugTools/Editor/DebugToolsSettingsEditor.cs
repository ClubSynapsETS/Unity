...
public class DebugToolsSettingsEditor : EditorWindow {
	...
	private void OnEnable() {
		...
		leftMenu = new GUIStyle();
		leftMenu.fixedWidth = 200;
	}
	...
	private void OnGUI() {
		...
		EditorGUILayout.BeginHorizontal();
		GUILayout.FlexibleSpace();
		if (GUILayout.Button("|         Clear Settings          |")) ResetSettingsFile();
		if (GUILayout.Button("|          Save Settings          |")) SaveSettingsFile();
		if (GUILayout.Button("|           Save Profile          |")) SaveSettingsProfile();
		GUILayout.FlexibleSpace();
...
//		settings.postLogFiles = EditorGUILayout.Toggle("POST log files", settings.postLogFiles);
	...
	void ShowKeyBindingSettings() {
		...
		settings.consoleKey = (KeyCode)EditorGUILayout.EnumPopup("Console key", settings.consoleKey);
	...
	void ShowInformation() {
		GUILayout.Label((Texture)EditorGUIUtility.Load("DebugTools/Icon.png"));
	...
	private void SaveSettingsFile() {
		...
		try {
			...
			string filePath = Application.streamingAssetsPath + "/DebugTools/";
